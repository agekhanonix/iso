...
function Canevas (canvasDiv, canvasWidth, canvasHeight, canvasId) {
    ...
    ** ** ========================================= ** */
    ...
    /* --- Mouse / Touch events                             -- */
    elCanvas.addEventListener((isMobile ? 'touchstart' : 'mousedown'), function(e){     // To toggle drawing mode 'ON'
        drawing = true;
        lastPos = getMousePos(elCanvas, e);
        if(firstTouch) {
            mousePos = getMousePos(elCanvas, e);
            firstTouch = false;
        }
    }, false);
    elCanvas.addEventListener((isMobile ? 'touchend' : 'mouseup'), function(e){         // To toggle drawing mode 'OFF'
        drawing = false;
        firstTouch = true;
    }, false);
    elCanvas.addEventListener((isMobile ? 'touchmove' : 'mousemove'), function(e) {     // To toggle mouse position, used in drawing
        mousePos = getMousePos(elCanvas, e);
    }, false);
    ...
    ** -- the screen                                -- */
    ...
    document.body.addEventListener("touchstart", function (e) {
        if (e.target.getAttribute('id') == canvasId) {
            e.preventDefault();
        }
    }, false);
